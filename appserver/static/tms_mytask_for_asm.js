require([
    ...
    SearchManager) {
        ...
        $(document).on('click','.toggle-tab',function(){
            ...
            // refreshiframe()
            task_id=tokensDefault.get('form.taskid')
            app_name=tokensDefault.get('env:app')
            $('#tms_mytask_iframe').attr('src',$(this).attr('iframe_str')+'?hideChrome=true&hideFilter=true&hideEdit=true&hideTitle=true&taskid='+task_id+'&env_app='+app_name)
        ...
        $(document).on('click','.show_task_detail',function(){
            ...
            $('#task_row1').hide();
            $('#task_row2').hide();
            $('.toggle-tab[iframe_str="tms_related_asmasset"]').click();
            tablestatus=$.trim(tokensDefault.get('table_status'));
            console.log(tablestatus)
            if(tablestatus=="未认领"){
