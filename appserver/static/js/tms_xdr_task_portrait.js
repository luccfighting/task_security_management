require([
    ...
    tokensDefault.on('change', function (nodevalue, options) {
    ...
    function custom_multipleDown(default_list, total_list) {
        ...
        $.each(default_list, function (index, val) {
            ...
            if (index == default_list.length - 1) {
                const drag_drop = require("splunkjs/mvc/drag_drop")
                const new_drag_drop = new drag_drop()
            }
        })
    ...
    $(document).on('click', '.btn-topsek-edit', function () {
        ...
        $.each(depart_list, function (key, item) {
            $('.un-topsek-value[value-key-en="department"]').append(`<option label="${item["key"]}" value="${item["value"]}"></option>`)
        })
    ...
    $(document).on('click', '.btn-topsek-disposal', function () {
        if ($(this).attr('disabled') == undefined) {
            ...
            $('.task-topsek-value[data-value="department"]').html('<option value="" label="请选择角色"></option>')
            const depart_list = get_depart_list()
            $.each(depart_list, function (key, item) {
                // console.log(item)
                $('.task-topsek-value[data-value="department"]').append('<option value="' + item["value"] + '" label="' + item["key"] + '"></option>')
            })
            ...
            $.each(user_list, function (key, item) {
                ...
                $('.task-topsek-value[data-value="username"]').append('<option department="' + item["role"] + '" user_id="' + item["id"] + '" value="' + item["account"] + '" label="' + item["name"] + '"></option>')
            })
        }
    })
    ...
    function get_depart_list() {
        var depart_list = [
            {
                "key": "运维人员",
                "value": "mss_backstage_user"
            },
            {
                "key": "审核人员",
                "value": "mss_backstage_power"
            },
            {
                "key": "系统管理员",
                "value": "mss_backstage_admin"
            }
        ]
        return depart_list
    }
